...
AWARA.Admin.prototype={
    ...
    user_details: function() {
        var self = this;
            $.ajax({
                url: self.base_url+"users-table/show-user-details",
                type: "GET",
                dataType: "JSON",
                beforeSend: function() {
                },
                success: function (data) {
                    ...
                    for(var i=0;i<data.length;i++) {
                        var row = "<tr><td>"+[i+1]+"</td><td>" + data[i].fname + "</td><td>" + data[i].lname + "</td><td>" + data[i].email +"</td><td>"+data[i].status+"</td><td><a href='admin/users/"+data[i].id+"'><button class='btn btn-primary'>View</button></a></td></tr>";
                        $("#users_data_json").append(row);
                    }
                    self.update_status();
                },
                complete: function () {
